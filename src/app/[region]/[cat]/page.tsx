...
export default function CatPage() {  
  ...
  return (  
    <div style={{  
      minHeight: '100vh', background: '#f4f6fb', padding: '16px',  
      fontFamily: "'Noto Sans KR', sans-serif"  
    }}>  
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>  
        ...
        <button  
          onClick={() => router.back()}  
          style={{  
            marginBottom: '14px', padding: '9px 18px',  
        ...
        <h1 style={{ fontSize: '20px', fontWeight: '800', color: '#1a1a2e', marginBottom: '2px' }}>  
          📍 {region} · {cat}  
        </h1>  
        <p style={{ fontSize: '12px', color: '#888', marginBottom: '14px' }}>  
          {selectedRegion2 ? selectedRegion2 : `${region} 전체 합산`} 기준 · {selectedSubCat ? selectedSubCat : '전체'} · POS 내림차순 · 단위: 백만  
        </p>  
        ...
        <div style={{  
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>  
            ...
            >  
              <option value="">전체 ({region} 지점 합산)</option>  
              {region2List.map((r2) => (  
            ...
            <div style={{  
              ...
              {(searchProduct || searchMaker || selectedRegion2 || selectedSubCat) && (  
                <button  
                  onClick={handleReset}  
                  style={{  
                    padding: '4px 10px', background: '#fff0f0', border: 'none',  
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>  
            ...
            <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '6px' }}>  
              <span style={{ fontSize: '11px', color: '#888' }}>표시</span>  
              <select  
                value={displayCount}  
        ...
        <div style={{ overflowX: 'auto' }} onClick={() => setShowProductSugg(false)}>  
          <table style={{  
            width: '100%', borderCollapse: 'collapse',  
            background: 'white', borderRadius: '14px',  
            overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.08)',  
            fontSize: '12px', minWidth: '700px'  
          }}>  
            <thead>  
              <tr style={{ background: '#1a1a2e', color: 'white' }}>  
                <th rowSpan={2} style={{ padding: '10px 8px', textAlign: 'center', borderRight: '1px solid #333', width: '36px' }}>No</th>  
                <th rowSpan={2} style={{ padding: '10px 8px', textAlign: 'left', borderRight: '1px solid #333' }}>자재명</th>  
                <th rowSpan={2} style={{ padding: '10px 8px', textAlign: 'center', borderRight: '1px solid #333', whiteSpace: 'nowrap' }}>제조사</th>  
                <th style={{ padding: '10px 8px', textAlign: 'center', borderRight: '1px solid #333', background: '#2d3a6b', whiteSpace: 'nowrap' }}>POS (백만)</th>  
                <th colSpan={3} style={{ padding: '10px 8px', textAlign: 'center', borderRight: '1px solid #333', background: '#1a4a7a' }}>MS</th>  
                <th colSpan={2} style={{ padding: '10px 8px', textAlign: 'center', background: '#1e4d8c' }}>판가 (원)</th>  
              </tr>  
              <tr style={{ background: '#2a2a3e', color: '#ccc', fontSize: '11px' }}>  
                <th style={{ padding: '8px 6px', textAlign: 'center', borderRight: '1px solid #3a3a5e', whiteSpace: 'nowrap' }}>25년</th>  
                <th style={{ padding: '8px 6px', textAlign: 'center', borderRight: '1px solid #3a3a5e', whiteSpace: 'nowrap' }}>영본MS</th>  
                <th style={{ padding: '8px 6px', textAlign: 'center', borderRight: '1px solid #3a3a5e', whiteSpace: 'nowrap' }}>지점MS</th>  
                <th style={{ padding: '8px 6px', textAlign: 'center', borderRight: '1px solid #3a3a5e', whiteSpace: 'nowrap' }}>영본比</th>  
                <th style={{ padding: '8px 6px', textAlign: 'center', borderRight: '1px solid #3a3a5e', whiteSpace: 'nowrap' }}>지점</th>  
                <th style={{ padding: '8px 6px', textAlign: 'center', whiteSpace: 'nowrap' }}>영본</th>  
              </tr>  
            </thead>  
            <tbody>  
              {displayedData.map((sku, idx) => {  
                ...
                return (  
                  ...
                  >  
                    <td style={{ padding: '9px 6px', textAlign: 'center', borderRight: '1px solid #eee' }}>  
                      <div style={{  
                    ...
                    <td style={{ padding: '9px 8px', borderRight: '1px solid #eee' }}>  
                      <div style={{ fontWeight: '700', color: '#1a1a2e', fontSize: '12px', lineHeight: '1.4' }}>  
                        {sku.pn}  
                      </div>  
                    ...
                    <td style={{ padding: '9px 6px', textAlign: 'right', borderRight: '1px solid #eee', fontWeight: '600', color: '#1a1a2e' }}>  
                      {sku.pos != null ? sku.pos.toFixed(0) : '-'}  
                    </td>  
                    <td style={{ padding: '9px 6px', textAlign: 'center', borderRight: '1px solid #eee' }}>  
                      <span style={{ fontWeight: '700', fontSize: '12px', color: '#38a169' }}>  
                        {msRefPercent}%  
                      </span>  
                    </td>  
                    <td style={{ padding: '9px 6px', textAlign: 'center', borderRight: '1px solid #eee' }}>  
                      <span style={{ fontWeight: '800', fontSize: '13px', color: isCJ ? '#667eea' : '#e53e3e' }}>  
                        {msPercent}%  
                      </span>  
                    </td>  
                    <td style={{ padding: '9px 6px', textAlign: 'center', borderRight: '1px solid #eee' }}>  
                      <span style={{ fontWeight: '800', fontSize: '12px', color: isSkuPositive ? '#3182ce' : '#e53e3e' }}>  
                        {isSkuPositive ? '▲' : '▼'} {Math.abs(msDiffSku * 100).toFixed(1)}%  
                      </span>  
                    </td>  
                    <td style={{ padding: '9px 6px', textAlign: 'right', borderRight: '1px solid #eee', fontWeight: '700', color: '#1a1a2e' }}>  
                      {sku.price != null && sku.price > 0 ? Math.round(sku.price).toLocaleString() : '-'}  
                    </td>  
                    <td style={{ padding: '9px 6px', textAlign: 'right', fontWeight: '600', color: '#555' }}>  
